...
with open('masked-data/classes.names') as f:
...
network = cv2.dnn.readNetFromDarknet('masked-data/yolov3_mask_test.cfg',
                                     'masked-data/tambahan/yolov3_mask_train_final.weights')
...
while True:
    ...
    if len(results) > 0:
        ...
        for i in results.flatten():
            ...
            if class_numbers[i] == 1:
                subprocess.call(["ffplay", "-nodisp", "-autoexit", "tts.mp3"])
                cv2.rectangle(frame, (x_min, y_min),
                          (x_min + box_width, y_min + box_height), (3, 14, 186), 3)
            elif class_numbers[i] == 0:
                cv2.rectangle(frame, (x_min, y_min),
                          (x_min + box_width, y_min + box_height), (40, 198, 31), 3)
            elif class_numbers[i] == 2:
                cv2.rectangle(frame, (x_min, y_min),
                          (x_min + box_width, y_min + box_height), (0, 165, 240), 3)
